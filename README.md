...
Puedes importar la colección `Ticketing_Reactive_API.postman_collection.json` o utilizar cURL:

**Crear un Evento (Concierto)**
```bash
...
**Consultar Disponibilidad de Asientos (Reactivo y Rápido)**
```bash
curl -X GET http://localhost:8080/api/events/{event_id}/availability
```

**Comprar Asientos e Iniciar Orden (Async)**
El endpoint es asíncrono; retorna un *202 Accepted* porque su trabajo es realizar una reserva rápida y mandar la compra real a la cola SQS.
```bash
curl -X POST http://localhost:8080/api/events/{event_id}/purchase \
-H 'Content-Type: application/json' \
-d '{"userId": "user-auth-token1212", "quantity": 2}'
```

**Ver el Estado de la Orden Final (Polling)**
Posteriormente puedes consultar el ID generado. Idealmente este valor pasará con el tiempo de `PENDING_CONFIRMATION` -> `SOLD`.
```bash
